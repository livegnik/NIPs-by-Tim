...
If this direction ends up making sense, it should become much easier to build apps that normally need a central server, without actually having one. Apps can rely on profiles, settings, and lists ending up the same across clients. Communities can carry their rules and moderation with them instead of locking them into one app. Markets can show payments, receipts, and dispute outcomes without asking users to trust a private database. Messaging and coordination tools can keep working when relays go down or networks split. Apps can prove who created something, what happened, or that something was paid for, without handing control to a single operator.

More generally, apps would have to guess less about how relays behave. They could handle failures more safely, recover state more reliably, and share data and trust signals across different clients in a way that stays consistent and understandable.
