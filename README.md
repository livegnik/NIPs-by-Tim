...
NIP draft discussion: https://github.com/nostr-protocol/nips/issues/2188
...
Nostr has a recurring class of bugs where "the protocol is simple" but implementations disagree on what "valid" means in practice. Some clients accept malformed tags and compensate later. Some relays enforce hidden constraints without describing them. Some code paths verify signatures early, others late. That inconsistency forces every app feature, threading, indexing, rendering, notifications, to become defensive code. This NIP stops that leak by making validation strict, ordered, and uniform so the ecosystem converges on one definition of valid input.

NIP-01 defines the event shape and cryptographic rules, but it does not define a strict execution model for validation. In practice, every client and relay already runs a validation pipeline, yet it is implicit, inconsistently documented, and different between implementations. The result is that two compliant stacks can see the same event and reach different conclusions about whether it is storable, renderable, or even properly identified. That divergence pushes complexity upward into app logic and creates duplicates, replay inconsistencies, and brittle edge-case handling.

The problem is visible in real discussions and bugs: small differences in JSON handling and id derivation lead to incompatible behavior and mismatched event ids (https://github.com/nostr-protocol/nips/issues/354), and libraries have shipped issues where relays reject events as "invalid event id" because they validated the same inputs differently (https://github.com/rust-nostr/nostr/issues/227). These failures are not about application behavior; they originate from inconsistent validation decisions earlier in the stack. This NIP makes that pipeline explicit so different implementations converge on the same behavior without changing the underlying event definition.
